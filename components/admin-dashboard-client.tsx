...
export function AdminDashboardClient({
    ...
    const carStatuses = ["available", "sold", "pending"] // New: Car status options
        ...
        return new Promise((resolve) => {
            ...
            reader.onload = (event) => {
                ...
                img.onload = () => {
                    ...
                    const MAX_HEIGHT = 900 // Max height for images
                    let width = img.width
    ...
    const handleSubmitAddCar = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        carFormData.set("custom_id_prefix", newCarData.custom_id_prefix!) // Add custom ID prefix
        carFormData.set("custom_id_number", newCarData.custom_id_number!) // Add custom ID number
        carFormData.set("status", newCarData.status || "available") // Add status, default to available
        ...
        startTransition(async () => {
            try {
                ...
                setNewCarData({ status: "available" }) // Reset with default status
                setSelectedImageFiles([])
                setTimeout(() => {
                    setUploadProgress(0)
                    setUploadMessage("")
    ...
    // Helper to calculate days since posted
                    ...
                    <form onSubmit={handleSubmitAddCar} className="space-y-4" ref={addCarFormRef}>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            ...
                            <div className="space-y-2">
                                ...
                                <Select
                                    name="status"
                                    value={newCarData.status || "available"} // Default to 'available'
